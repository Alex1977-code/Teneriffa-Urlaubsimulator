...
const Game = (() => {
  ...
  function neuerUrlaub(cfg) {
    ...
    logEintrag('🛬', `Ankunft auf Teneriffa! Transfer nach ${D.REGIONEN[cfg.region].name.split(' (')[0]}, Koffer ins ${hotel.name} ${hotel.sterne} – der Urlaub beginnt am Nachmittag.`);
    runSpeichern();
    return run;
  }
  ...
  function effektiveEnergie(act) {
    if (act.energieMitSchuhen !== undefined && run.items.includes('wanderschuhe'))
  ...
  function aktivitaetAusfuehren(actId) {
    ...
    const dErholung = Math.round(skaliere(act.erholung || 0) * (act.erholung > 0 ? erholungsCap : 1));
    ...
    if (sonnenPunkte > 0) {
      ...
      if (run.items.includes('sonnencreme')) brand *= 0.3;
    ...
    let ereignis = null;
    if (Math.random() < 0.34) {
      const ctx = { act, zone: act.zone, wetterId, run, hops: anzahlHops };
